...
<div class="container px-4 py-5">
    <h1 class="mt-5">Изменение заправки</h1>
    ...
    <form method="post" action="{{route('refilling.update', $Refilling->id)}}">
        ...
        <select name="driver-id" id="driver-id" class="form-select form-select-lg mb-3" aria-label="Водитель">
            ...
            <option value="{{$User->id}}"
                    @if ($Refilling->driver->id == $User->id)
                selected
                @endif
        ...
        <select name="petrol-stations-id" id="petrol-stations-id" class="form-select form-select-lg mb-3" aria-label="Название АЗС">
            <option value="0">Название АЗС</option>
            @foreach($PetrolStations as $PetrolStation)
            <option value="{{$PetrolStation->id}}"
                    @if ($Refilling->petrolStation->id == $PetrolStation->id)
                selected
                @endif
        ...
        <div class="form-floating mb-3">
            <input type="date" name="date-car-refueling" id="date-car-refueling" placeholder="Дата заправки" class="form-control form-control-lg" value="{{$Refilling->dateEdit}}">
            <label for="date-car-refueling">Дата заправки</label>
        </div>
        <div class="form-floating mb-3">
            <input type="number" min="10" max="500" name="num-liters-car-refueling" id="num-liters-car-refueling" placeholder="Количество литров" class="form-control form-control-lg" value="{{$Refilling->num_liters_car_refueling}}">
            <label for="num-liters-car-refueling">Количество литров</label>
        </div>
        <div class="form-floating mb-3">
            <input type="number" step="any" name="price-car-refueling" id="price-car-refueling" placeholder="Цена за 1 литр" class="form-control form-control-lg" value="{{$Refilling->price_car_refueling}}">
            <label for="price-car-refueling">Цена за 1 литр</label>
        </div>
        <div class="form-floating mb-3">
            <input type="text" name="cost-car-refueling" id="cost-car-refueling" placeholder="Стоимость заправки" class="form-control form-control-lg" value="{{$Refilling->cost_car_refueling}}" disabled>
